...
export default function Admin() {
  ...
  async function deleteQuiz(quiz) {
    if (questions.some(q => q.quizId === quiz.id)) { ok('⚠️ Supprime d\'abord les questions'); return; }
  ...
  async function saveCat(id, data) {
    ...
    setEditItem(null); ok('✓ Catégorie modifiée');
  }
  ...
  async function saveTheme(id, data) {
    ...
    setEditItem(null); ok('✓ Thème modifié');
  }

  // ── Catégories dispo (statiques + firebase) ───────────────────────────────────

  const allCats = [
    ...Object.values(CATEGORIES).map(c => ({ id: c.id, label: c.label, icon: c.icon, isBuiltin: true })),
    ...cats.map(c => ({ ...c, isBuiltin: false })),
  ];

  const allThemes = [
    ...Object.values(CATEGORIES).flatMap(cat =>
      Object.values(cat.themes).map(t => ({ id: t.id, label: t.label, icon: t.icon, catId: cat.id, isBuiltin: true }))
    ),
    ...themes.map(t => ({ ...t, isBuiltin: false })),
  ];
  ...
  return (
    <div className="page">
      ...
      <div style={{ background: 'linear-gradient(135deg, #1a1040, #2d1a5e)', border: '1px solid #4a3580', borderRadius: 16, padding: 16, marginBottom: 14 }}>
        ...
        <div style={{ color: 'var(--muted)', fontSize: 12 }}>{questions.length} questions · {quizzes.length} quiz · {allCats.length} catégories</div>
      </div>
        ...
        <div>
          ...
          {filtered.length === 0 && !loading && (
            <div style={{ textAlign: 'center', padding: '32px 16px', color: 'var(--muted)' }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 6 }}>Aucune question en base</div>
              <p style={{ fontSize: 12 }}>Va dans <strong>Ajouter</strong> pour importer ton CSV ou ajouter des questions manuellement.</p>
            </div>
          )}
        </div>
      )}
      ...
      {!loading && tab === 'quizzes' && (
        <div>
          <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 12 }}>{quizzes.length} quiz en base</div>
          {quizzes.map(quiz => {
                    ...
                    <div style={S.row}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                          ...
                          <span style={{ fontSize: 11, color: 'var(--muted)' }}>{quizQuestions.length} questions</span>
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 2, flexShrink: 0 }}>
                ...
                {expandedQuiz === quiz.id && (
                  <div style={{ borderTop: '1px solid var(--s2)', background: 'rgba(0,0,0,.15)' }}>
                    {quizQuestions.length === 0
                      ? <div style={{ padding: '12px 13px', fontSize: 12, color: 'var(--muted)' }}>Aucune question liée à ce quiz.</div>
          ...
          {quizzes.length === 0 && (
            <div style={{ textAlign: 'center', padding: '32px 16px', color: 'var(--muted)' }}>
              <div style={{ fontSize: 36, marginBottom: 10 }}>📭</div>
              <div style={{ fontSize: 14, marginBottom: 6 }}>Aucun quiz en base</div>
            </div>
          )}
      ...
      {!loading && tab === 'ajouter' && (
        <div>
              ...
              <div style={S.grid2}>
                {qOpts.map((opt, i) => (
                  <div key={i} style={{ position: 'relative' }}>
                    ...
                    <button type="button" onClick={() => setQAns(i)} style={{ position: 'absolute', right: 7, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', fontSize: 12, cursor: 'pointer', color: 'var(--cyan)', opacity: qAns===i?1:0.3 }}>✓</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={S.divider} />
          <div style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: 14, marginBottom: 6 }}>Import CSV</div>
          ...
          >
            ...
            <input id="csv-in" type="file" accept=".csv" style={{ display: 'none' }} onChange={parseCSV} />
          </div>
          {csvRows.length > 0 && (
            <div style={{ background: 'var(--s2)', borderRadius: 10, padding: 12 }}>
              ...
              <button className="btn btn-primary btn-full" style={{ marginTop: 10, fontFamily: "'Raleway', sans-serif", fontWeight: 700 }} onClick={importCSV}>
      ...
      {!loading && tab === 'categories' && (
        <div>
          <div style={S.secLabel}>Catégories intégrées (structure)</div>
      ...
      {!loading && tab === 'themes' && (
        <div>
          <div style={S.secLabel}>Thèmes intégrés (structure)</div>
          {Object.values(CATEGORIES).flatMap(cat =>
